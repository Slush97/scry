...
pub struct IncrementalRenderer {
    ...
    /// Cached GPU context (lazy-initialized).
    #[cfg(feature = "gpu")]
...
impl IncrementalRenderer {
    ...
    pub fn render_canvas(&mut self, canvas: &PixelCanvas) -> Result<(), PixelCanvasError> {
        ...
        // Check cache — if valid and not halfblock protocol, try incremental
        if !self.skip_cache && self.cache.is_valid(content_hash) {
            if is_halfblock {
                let cached = self.cache.get(content_hash).cloned();
                if let Some(ref pixmap) = cached {
                    self.render_halfblock_stdout(pixmap)?;
                }
            }
            // For protocol backends with existing handle, nothing to do (same frame).
            return Ok(());
        }

        // Rasterize into cached pixmap
        ...
        let Some(pixmap) = pixmap_opt else {
            return Err(PixelCanvasError::PixmapCreation(
                "failed to allocate pixmap".to_string(),
            ));
        };
        ...
        if is_halfblock {
            let cached = self.cache.get(store_hash).cloned();
            if let Some(ref pixmap) = cached {
                self.render_halfblock_stdout(pixmap)?;
            }
            return Ok(());
        ...
        if let Some(ref old_handle) = self.current_handle {
            if !dirty_tiles.is_empty() {
                let pixmap = self.cache.get(store_hash).expect("cache validated above");
