//! pixelchart CLI — generate pixel-perfect charts from JSON.
//!
//! ```bash
//! # From stdin
...
//!
//! # Save to PNG
//! echo '{"type":"bar","data":{"labels":["Q1","Q2","Q3"],"values":[100,200,150]}}' \
//!   | pixelchart render --output chart.png
//!
//! # Show an existing PNG inline
...
/// pixelchart — pixel-perfect terminal charts from JSON
#[derive(Parser, Debug)]
#[command(name = "pixelchart", version, about)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn main() {
    ...
    let result = match cli.command {
        Commands::Render {
            ...
            r#type, data, title, x_label, y_label, width, height, theme, output,
        ),
        ...
        Commands::Example {
            ...
            theme,
            output,
        } => cmd_example(chart_type, width, height, theme, output),
        Commands::Plot {
            r#type,
            csv,
            x,
            ...
            no_header, y_min, y_max, x_min, x_max, bins, skip_rows, max_rows, sort, no_bg,
        ),
...
fn cmd_render(
    ...
    // Build chart
    let chart = spec.into_chart()?;

    // Render to PNG
    let png_data = pixelchart::export::render_to_png(&chart, w, h)?;
...
fn cmd_example(
    ...
    let theme = match theme_name.as_str() {
        "light" => pixelchart::theme::Theme::light(),
        _ => pixelchart::theme::Theme::dark(),
    };
    ...
    for (i, ct) in types.iter().enumerate() {
        let chart = examples::build_example(*ct, theme.clone());
        let png_data = pixelchart::export::render_to_png(&chart, width, height)?;
...
fn cmd_info() -> Result<(), String> {
    println!("pixelchart — pixel-perfect terminal charts");
    println!();
    ...
    println!("  --theme dark|light  Color theme (default: dark)");
