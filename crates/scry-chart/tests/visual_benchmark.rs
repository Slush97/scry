...
fn type_gallery() -> Vec<(&'static str, Chart)> {
    vec![
        (
            "Line",
            Charts::line(&[1.0, 4.0, 2.0, 8.0, 5.0, 3.0, 7.0, 6.0])
                .title("Line Chart")
                .x_label("Time")
                .y_label("Value")
                .theme(Theme::dark())
        ...
        (
            "Area (filled)",
            Charts::area(&[3.0, 7.0, 4.0, 9.0, 6.0, 8.0, 5.0])
                .title("Area Chart")
                .x_label("Time")
                .y_label("Revenue")
                .theme(Theme::dark())
        ...
        (
            "Sparkline",
            Charts::sparkline(&[3.0, 7.0, 4.0, 8.0, 2.0, 9.0, 5.0, 6.0, 3.0]).build(),
        ),
    ]
}
            ...
            let series: Vec<Series> = (0..8)
                .map(|s| {
                    let values: Vec<f64> = (0..20)
                        .map(|i| (i as f64 * 0.3 + s as f64 * 0.5).sin() * 10.0 + s as f64 * 5.0)
                        .collect();
...
fn extreme_values() -> Vec<(&'static str, Chart)> {
    vec![
        ...
        (
            ...
            .title("All Negative — Bar Baseline")
            .y_label("Loss ($K)")
            .theme(Theme::dark())
            .build(),
        ),
...
"#
    ...
    fn table(&mut self, headers: &[&str], rows: &[Vec<String>]) {
        ...
        for h in headers {
            write!(self.buf, "<th>{h}</th>").unwrap();
        }
        write!(self.buf, "</tr></thead><tbody>").unwrap();
        for row in rows {
            write!(self.buf, "<tr>").unwrap();
            for cell in row {
                write!(self.buf, "<td>{cell}</td>").unwrap();
            }
            write!(self.buf, "</tr>").unwrap();
        }
...
fn main() {
    ...
    let total =
        gallery.len() + themes.len() + text.len() + density.len() + extremes.len() + sizes.len();
    ...
    report.table(
        ...
        &[
            ...
            vec![
                "M3".into(),
                "WCAG AA Contrast".into(),
                "Material".into(),
                "Text:background ≥ 4.5:1 contrast ratio".into(),
            ],
            vec![
                "M4".into(),
                "Bar Zero Baseline".into(),
                "Material".into(),
