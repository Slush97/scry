...
//!
//! These structs define the JSON schema that tools, frontends, and users produce
//! to describe a chart. The spec is parsed from JSON and converted to a
//! [`Chart`](crate::chart::Chart) for rendering.
//!
//! # Example
//!
//! ```ignore
//! use scry_chart::json_spec::JsonChartSpec;
//!
//! let json = r#"{"type":"line","data":{"y":[1,4,2,8,5]},"title":"My Chart"}"#;
//! let spec: JsonChartSpec = serde_json::from_str(json).unwrap();
...
pub struct ChartData {
    ...
    // --- Gantt ---
    /// Task entries for Gantt chart
    pub tasks: Option<Vec<GanttTaskSpec>>,
}
...
/// A Gantt chart task entry.
#[derive(Debug, Deserialize)]
pub struct GanttTaskSpec {
    /// Task label
    pub label: String,
...
impl JsonChartSpec {
    ...
    pub fn into_chart(self) -> Result<Chart, String> {
        ...
        let chart = match self.chart_type {
            ...
            ChartType::Gantt => self.build_gantt(theme)?,
        };
    ...
    fn build_gantt(&self, theme: Theme) -> Result<Chart, String> {
        ...
        let tasks: Vec<GanttTask> = task_specs
            .iter()
            .map(|t| {
                let mut task = GanttTask::new(&t.label, t.start, t.end);
                if let Some(ref g) = t.group {
                    task = task.group(g);
                }
                if let Some(p) = t.progress {
                    task = task.progress(p);
                }
                task
            })
            .collect();
...
///
/// Returns `Theme::dark()` for unrecognized or absent names.
