...
//! rendering via `scry-engine` and overlay text labels via ratatui.
...
fn render_text_overlays(buf: &mut Buffer, area: Rect, overlays: &[TextOverlay], font: FontSize) {
    for overlay in overlays {
        ...
        if overlay.rotation_deg >= 89.0 {
            // 90° rotation: render characters bottom-to-top (counter-clockwise).
            // This is the standard convention for Y-axis labels: text reads
            // upward from bottom to top.

            // Compute vertical start based on alignment:
            // Center-aligned text should be vertically centered at abs_y.
            let start_y = match overlay.align {
                TextAlign::Center => abs_y.saturating_sub(text_len.div_ceil(2)),
                TextAlign::Right => abs_y.saturating_sub(text_len),
                TextAlign::Left => abs_y,
            };

            // Reverse the character order so text reads bottom-to-top
            // (counter-clockwise 90° rotation).
            for (i, ch) in text.chars().rev().enumerate() {
                let cy = start_y + i as u16;

                if cy >= area.y + area.height || abs_x >= area.x + area.width {
                    break;
                }
                if abs_x >= area.x && cy >= area.y {
                    if let Some(cell) = buf.cell_mut(Position::new(abs_x, cy)) {
                        cell.set_char(ch);
                        cell.set_style(style);
                    }
                }
            }
        } else if overlay.rotation_deg > 0.0 {
            // Diagonal rotation (e.g. 45°): stack characters downward from
            // the anchor, stepping right for each row. For right-aligned
            // rotated X-tick labels, the anchor is at the tick mark position.
            //
            // NOTE: Diagonal text in a character-cell terminal produces a
            // "staircase" effect because each glyph occupies exactly one
            // cell. True sub-cell rotation is only possible in the PNG/SVG
            // export paths; the terminal widget can only approximate it by
            // placing one character per row+column step.

            // Right-aligned: shift left by text length so text ends at anchor
            let start_x = match overlay.align {
                TextAlign::Right => abs_x.saturating_sub(text_len),
                TextAlign::Center => abs_x.saturating_sub(text_len.div_ceil(2)),
