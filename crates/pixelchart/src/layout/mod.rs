...
use crate::axis::{self, AxisConfig, AxisSide};
...
fn proportional_margin(w: u32, h: u32) -> f32 {
    let short = (w.min(h)) as f32;
    (short * MARGIN_FRAC).max(MIN_MARGIN).min(24.0)
}

/// Compute Y-axis width (for tick labels) based on canvas width.
fn proportional_y_axis_width(w: u32) -> f32 {
    let w = w as f32;
    (w * 0.12).max(30.0).min(70.0)
}

/// Compute title height based on canvas height.
fn proportional_title_height(h: u32) -> f32 {
    let h = h as f32;
    (h * 0.06).max(14.0).min(30.0)
}

/// Compute X-axis tick label height based on canvas height.
fn proportional_x_tick_height(h: u32) -> f32 {
    let h = h as f32;
...
fn proportional_y_label_width(w: u32) -> f32 {
    let w = w as f32;
    (w * 0.03).max(8.0).min(16.0)
}
...
pub(crate) struct RenderContext {
    /// The pixel canvas being drawn to.
    pub canvas: PixelCanvas,
...
impl RenderContext {
    ...
    pub fn new(config: &ChartConfig, w: u32, h: u32) -> Self {
        let plot = compute_plot_area(w, h, config);
    ...
    pub fn draw_axes(&mut self, config: &ChartConfig, x_scale: &LinearScale, y_scale: &LinearScale) {
        let (canvas, x_ticks) = axis::draw_axis(
            std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1)),
            self.plot,
            x_scale,
            &axis_config_from_theme(config, AxisSide::Bottom),
        );
        self.canvas = canvas;

        let (canvas, y_ticks) = axis::draw_axis(
            std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1)),
            self.plot,
            y_scale,
            &axis_config_from_theme(config, AxisSide::Left),
        );
        self.canvas = canvas;

        self.add_tick_overlays(&x_ticks, &y_ticks, config.theme.text_color);
    }

    /// Draw only the Y axis (for categorical X charts like bar/boxplot).
    pub fn draw_y_axis(&mut self, config: &ChartConfig, y_scale: &LinearScale) -> Vec<(f32, String)> {
        let (canvas, y_ticks) = axis::draw_axis(
            std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1)),
            self.plot,
            y_scale,
    ...
    pub fn draw_x_axis_line(&mut self, config: &ChartConfig) {
        ...
        self.canvas = std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1))
            .line(px, py + ph, px + pw, py + ph)
            .color(config.theme.axis_color)
            .width(config.theme.axis_width)
            .done();
    }

    /// Draw reference lines on the canvas.
    pub fn draw_reference_lines(&mut self, config: &ChartConfig, x_scale: &LinearScale, y_scale: &LinearScale) {
        let (px, py, pw, ph) = self.plot;
        ...
        for rl in &config.h_lines {
            ...
            if y >= py && y <= py + ph {
                let mut builder = std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1))
                    .line(px, y, px + pw, y)
                    .color(rl.color)
                    .width(rl.width);
                if rl.dashed {
                    builder = builder.dash(dash.clone());
                }
                self.canvas = builder.done();
            }
        }
    ...
    pub fn add_common_overlays(&mut self, config: &ChartConfig) {
        ...
        if let Some(ref label) = config.x_label {
            let x_tick_h = proportional_x_tick_height(h);
            let x_label_h = proportional_x_label_height(h);
            self.overlays.push(TextOverlay {
                x_px: px + pw / 2.0,
                y_px: py + ph + x_tick_h + x_label_h / 2.0,
                text: label.clone(),
                color: config.theme.text_color,
                align: TextAlign::Center,
            });
    ...
    pub fn draw_annotations(&mut self, config: &ChartConfig, x_scale: &LinearScale, y_scale: &LinearScale) {
        for ann in &config.annotations {
            ...
            if let Some(bg) = ann.style.background {
                self.canvas = std::mem::replace(&mut self.canvas, PixelCanvas::new(1, 1))
                    .rect(text_x - 2.0, text_y - 2.0, 60.0, 16.0)
                    .fill(bg)
                    .corner_radius(3.0)
                    .done();
            }
    ...
    pub fn draw_trend_line(&mut self, x_vals: &[f64], y_vals: &[f64], x_scale: &LinearScale, y_scale: &LinearScale, color: Color) {
        ...
        if denom.abs() < f64::EPSILON { return; }
...
pub(crate) fn compute_plot_area(w: u32, h: u32, config: &ChartConfig) -> (f32, f32, f32, f32) {
    let margin = proportional_margin(w, h);
    let y_axis_w = proportional_y_axis_width(w);
    let title_h = if config.title.is_some() {
        proportional_title_height(h)
    } else {
        0.0
    };
    let y_label_w = if config.y_label.is_some() {
        proportional_y_label_width(w)
    } else {
        0.0
    };
    let x_tick_h = proportional_x_tick_height(h);
...
/// Resolve the effective x extent: use config override if set, else data extent.
pub(crate) fn resolve_x_extent(config: &ChartConfig, data_extent: (f64, f64)) -> (f64, f64) {
    config.x_range.unwrap_or(data_extent)
}

/// Resolve the effective y extent: use config override if set, else data extent.
pub(crate) fn resolve_y_extent(config: &ChartConfig, data_extent: (f64, f64)) -> (f64, f64) {
    config.y_range.unwrap_or(data_extent)
}

/// Take canvas out of RenderContext for move-based APIs, replacing with a placeholder.
pub(crate) fn take_canvas(ctx: &mut RenderContext) -> PixelCanvas {
    std::mem::replace(&mut ctx.canvas, PixelCanvas::new(1, 1))
}
