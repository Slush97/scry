...
///
/// Must stay in sync with `AVG_CHAR_WIDTH_11PX` in `layout/mod.rs`.
...
fn auto_skip_labels(
    ...
    let label_span = if is_horizontal {
        ...
        // Y-axis: spacing is vertical. Each label needs ~14px line height + gap.
        14.0 + MIN_LABEL_GAP
    };

    let spacing = axis_length / pairs.len().max(1) as f32;

    // If labels fit comfortably, keep all
    if spacing >= label_span {
        return pairs;
    }

    // Calculate skip factor: how many labels to skip to avoid overlap
    let skip = (label_span / spacing).ceil() as usize;
    if skip <= 1 {
        return pairs;
    }
    ...
    pairs
        .into_iter()
        .enumerate()
        .filter(|(i, _)| i % skip == 0 || *i == 0 || *i == total - 1)
        .map(|(_, p)| p)
        .collect()
}
...
mod tests {
    ...
    fn auto_skip_preserves_endpoints() {
        ...
        let result = auto_skip_labels(pairs, 200.0, true, LabelRotation::Horizontal);
        // First and last labels must always be preserved
        assert_eq!(result.first().unwrap().1, "label_0");
        assert_eq!(result.last().unwrap().1, "label_9");
    }
    ...
    fn auto_skip_y_axis_uses_vertical_spacing() {
        ...
        let pairs: Vec<(f64, String)> = (0..20)
            ...
            .collect();
        let result = auto_skip_labels(pairs, 200.0, false, LabelRotation::Horizontal);
        // With ~14+5=19px per label and 200px total, can fit ~10, so should skip some
        assert!(
            result.len() < 20,
            "Y-axis auto-skip should skip some labels"
        );
        assert_eq!(result.first().unwrap().1, "0");
        assert_eq!(result.last().unwrap().1, "1900");
    }
    ...
    fn auto_skip_with_angle_rotation() {
        ...
        let pairs: Vec<(f64, String)> = (0..10)
            ...
            .collect();
        let result_horiz = auto_skip_labels(pairs.clone(), 300.0, true, LabelRotation::Horizontal);
        let result_angle = auto_skip_labels(pairs, 300.0, true, LabelRotation::Angle(60.0));
