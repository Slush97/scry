...
//! let svg = pixelchart::svg_export::render_to_svg(&chart, 800, 500);
    ...
    match cmd {
        ...
        DrawCommand::Group {
            ...
            if (*opacity - 1.0).abs() > 0.001 {
                write!(group_attrs, r#" opacity="{:.2}""#, opacity).unwrap();
            }

            write!(body, "<g{}>", group_attrs).unwrap();
            body.push('\n');
        ...
        // Path commands from arbitrary bezier paths — emit as SVG <path>
        DrawCommand::Path { path, style } => {
            let fill_attr = svg_fill_attr(style, defs, grad_id);
            let stroke_attr = svg_stroke_attr(style);
            let d = path_data_to_svg_d(path);
            write!(body, r#"<path d="{}"{}{}/>"#, d, fill_attr, stroke_attr).unwrap();
            body.push('\n');
        }

        // Silently skip commands we don't handle in SVG output (e.g. Text
        // from the upstream `text` feature).
        _ => {}
    }
}
...
fn emit_arc(
    ...
    let large_arc = if sweep_angle.abs() > std::f32::consts::PI {
        1
    } else {
        0
    };
...
fn path_data_to_svg_d(path_data: &ratatui_pixelcanvas::scene::command::PathData) -> String {
    ...
    for seg in path.segments() {
        match seg {
            tiny_skia::PathSegment::MoveTo(pt) => {
                write!(d, "M {:.1} {:.1} ", pt.x, pt.y).unwrap();
            }
            tiny_skia::PathSegment::LineTo(pt) => {
                write!(d, "L {:.1} {:.1} ", pt.x, pt.y).unwrap();
            }
            tiny_skia::PathSegment::QuadTo(p1, p2) => {
                write!(d, "Q {:.1} {:.1} {:.1} {:.1} ", p1.x, p1.y, p2.x, p2.y).unwrap();
            }
...
fn emit_text_overlay(body: &mut String, overlay: &TextOverlay) {
    ...
    write!(
        body,
        r#"<text x="{:.1}" y="{:.1}" font-family="Inter, system-ui, sans-serif" font-size="{:.0}" font-weight="{}" fill="{}" text-anchor="{}"{}>{}</text>"#,
...
fn svg_fill_attr(style: &ShapeStyle, defs: &mut String, grad_id: &mut u32) -> String {
    match &style.fill {
        ...
        Some(FillStyle::LinearGradient(grad)) | Some(FillStyle::RadialGradient(grad)) => {
...
fn svg_stroke_attr(style: &ShapeStyle) -> String {
    match &style.stroke {
        None => String::new(),
        Some(s) => svg_stroke_style_attr(s),
    }
}
...
fn svg_stroke_style_attr(stroke: &StrokeStyle) -> String {
    ...
    if let Some(ref dash) = stroke.dash {
        let pattern = svg_dash_pattern(dash);
        write!(attrs, r#" stroke-dasharray="{}""#, pattern).unwrap();
        if dash.offset.abs() > 0.001 {
            write!(attrs, r#" stroke-dashoffset="{:.1}""#, dash.offset).unwrap();
        }
    }
...
mod tests {
    ...
    fn save_svg_creates_file() {
        ...
        let path = std::env::temp_dir().join("pixelchart_test_save.svg");
