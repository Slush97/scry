...
//!
//! Run: cargo test -p pixelchart --test axis_diagnostic -- --nocapture

use pixelchart::chart::Chart;
use pixelchart::layout::{self, RenderedChart, TextAlign, TextOverlay};
...
fn measure_overlay(overlay: &TextOverlay) -> TextBBox {
    ...
    TextBBox {
        ...
        role: String::new(), // filled in by classify
    }
}
...
fn classify_overlays(
    ...
    rendered
        ...
        .map(|o| {
            ...
            // Y-tick labels: right-aligned, left of plot area
            if o.align == TextAlign::Right && o.x_px <= px + 5.0 {
                bbox.role = "y_tick".to_string();
                return bbox;
            }

            // X-tick labels: center-aligned, below plot area
            if o.align == TextAlign::Center && o.y_px > py + ph * 0.8 {
