...
//! Native system-info collector for the fetch splash.
//!
//! All data is gathered from `/proc`, `/etc`, and environment variables.
//! The only subprocess spawned is a package-manager query (pacman / dpkg /
...
impl SysInfo {
    ...
    /// Look up a field value by its config ID.
    pub(super) fn field_value(&self, id: &str) -> &str {
        match id {
            "os" => &self.os,
            "kernel" => &self.kernel,
            "uptime" => &self.uptime,
    ...
    /// Detect the distro ID (e.g. `"arch"`, `"ubuntu"`, `"fedora"`).
    pub(super) fn distro_id() -> String {
