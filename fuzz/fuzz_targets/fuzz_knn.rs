//! Fuzz target: KNN classifier + regressor.
//!
//! Exercises KnnClassifier and KnnRegressor on fuzz-derived datasets.
//! Tests that no combination of inputs causes panics.
...
fuzz_target!(|data: &[u8]| {
    ...
    cursor += 1;
    let k = (data[cursor] % 5).max(1) as usize;
    cursor += 1;
    ...
    match dispatch {
        0 => {
            let mut model = KnnClassifier::new().k(k);
        ...
        _ => {
            let mut model = KnnRegressor::new().k(k);
            if model.fit(&dataset).is_ok() {
                let _ = model.predict(&test_sample);
            }
        }
