...
fn main() {
    ...
    for (name, builder) in &variants {
        print!("  ⟐ {name}... ");
        let scene = builder();
        let pixmap = SdfRenderer::render_to_pixmap(&scene, size, size, 0.0)
            .expect("SDF render failed");
...
fn build_obsidian_mirror() -> SdfScene {
    ...
    let mut scene = SdfScene::new()
        .object(SdfObject::new(SdfShape::Plane, Material::Solid {
            color: Color::from_rgba8(8, 8, 14, 255),
            reflectivity: 0.35, specular: 48.0,
        }))
        // Mirror disc
        .object(SdfObject::new(
            SdfShape::Cylinder { radius: 2.0, half_height: 0.06 },
            Material::Solid { color: obsidian, reflectivity: 0.85, specular: 128.0 },
        ).at(Vec3::new(0.0, 1.8, 0.0)).rotate(Vec3::new(1.0, 0.0, 0.0), 0.52))
        // Rim
        .object(SdfObject::new(
            SdfShape::Torus { major: 2.0, minor: 0.1 },
            Material::Solid { color: silver, reflectivity: 0.5, specular: 64.0 },
        ).at(Vec3::new(0.0, 1.8, 0.0)).rotate(Vec3::new(1.0, 0.0, 0.0), 0.52))
        // Handle
        .object(SdfObject::new(
            SdfShape::Capsule { radius: 0.18, half_height: 1.2 },
            Material::Solid { color: gold_warm, reflectivity: 0.3, specular: 48.0 },
        ).at(Vec3::new(0.0, -0.2, 0.7)))
        // Pommel
        .object(SdfObject::new(
            SdfShape::Sphere { radius: 0.28 },
            Material::Solid { color: gold_warm, reflectivity: 0.4, specular: 64.0 },
        ...
        // Rainbow disc
        .object(SdfObject::new(
            SdfShape::Cylinder { radius: 1.6, half_height: 0.008 },
            Material::Rainbow { saturation: 0.85, lightness: 0.45, hue_offset: 0.0, specular: 48.0 },
        ).at(Vec3::new(0.0, 1.84, 0.0)).rotate(Vec3::new(1.0, 0.0, 0.0), 0.52));

    for (x, height, z, color) in &pillar_data {
        scene = scene.object(SdfObject::new(
            SdfShape::RoundedBox { half_extents: Vec3::new(0.22, height / 2.0, 0.18), radius: 0.07 },
            Material::Solid { color: *color, reflectivity: 0.2, specular: 48.0 },
    ...
    for (pos, radius, color) in &accents {
        scene = scene.object(SdfObject::new(
            ...
            Material::Solid { color: *color, reflectivity: 0.6, specular: 128.0 },
        ).at(*pos));
    }

    scene
        .light(SdfLight::new(Vec3::new(5.0, 10.0, 5.0), Color::from_rgba8(255, 240, 210, 255), 0.85))
        .light(SdfLight::new(Vec3::new(-6.0, 7.0, 3.0), Color::from_rgba8(100, 130, 220, 255), 0.4))
        .light(SdfLight::new(Vec3::new(0.0, 6.0, -5.0), Color::from_rgba8(180, 160, 220, 255), 0.5))
        .camera(SdfCamera::new(Vec3::new(0.0, 4.5, 8.0), Vec3::new(0.0, 1.6, 0.0), 44.0))
        .max_bounces(3)
        .ambient(0.05)
        .sky_color(deep_sky)
}
...
fn build_recursive_eye() -> SdfScene {
    ...
    SdfScene::new()
        ...
        .object(SdfObject::new(SdfShape::Plane, Material::Checkerboard {
        ...
        .object(SdfObject::new(
            SdfShape::Mandelbulb { power: 8.0, iterations: 12 },
            Material::Solid {
                color: iris_hue,
                reflectivity: 0.4,
                specular: 96.0,
        ...
        .object(SdfObject::new(
            SdfShape::Torus { major: 2.2, minor: 0.08 },
            Material::Solid { color: gold, reflectivity: 0.6, specular: 96.0 },
        ).at(Vec3::new(0.0, 2.0, 0.0))
         .rotate(Vec3::new(1.0, 0.0, 0.0), FRAC_PI_4))
        // Outer ring 2 — perpendicular
        .object(SdfObject::new(
            SdfShape::Torus { major: 2.5, minor: 0.06 },
            Material::Rainbow { saturation: 0.9, lightness: 0.5, hue_offset: 0.0, specular: 64.0 },
        ).at(Vec3::new(0.0, 2.0, 0.0))
         .rotate(Vec3::new(0.0, 0.0, 1.0), FRAC_PI_4))
        // Outer ring 3 — tilted
        .object(SdfObject::new(
            SdfShape::Torus { major: 2.8, minor: 0.05 },
            Material::Solid {
                color: Color::from_rgba8(160, 80, 200, 255),
                reflectivity: 0.5, specular: 64.0,
            },
        ).at(Vec3::new(0.0, 2.0, 0.0))
        ...
        .light(SdfLight::new(Vec3::new(6.0, 8.0, 6.0), Color::from_rgba8(255, 220, 180, 255), 0.8))
        .light(SdfLight::new(Vec3::new(-5.0, 10.0, -3.0), Color::from_rgba8(120, 160, 255, 255), 0.6))
        .light(SdfLight::new(Vec3::new(0.0, 0.5, 8.0), Color::from_rgba8(200, 100, 255, 255), 0.3))
...
fn build_loom_of_threads() -> SdfScene {
    ...
    for (pos, tilt) in &spindle_positions {
        scene = scene.object(SdfObject::new(
            SdfShape::Capsule { radius: 0.12, half_height: 1.8 },
            Material::Solid { color: copper, reflectivity: 0.4, specular: 64.0 },
        ).at(*pos).rotate(Vec3::new(0.0, 0.0, 1.0), *tilt));
    }
    ...
    for angle in &thread_angles {
        scene = scene.object(SdfObject::new(
            SdfShape::Torus { major: 3.0, minor: 0.02 },
            Material::Solid {
                color: Color::from_rgba8(200, 160, 80, 255),
                reflectivity: 0.3, specular: 48.0,
            },
        ).at(Vec3::new(0.0, 2.5, 0.0))
    ...
    scene
        .light(SdfLight::new(Vec3::new(4.0, 10.0, 6.0), Color::from_rgba8(255, 200, 150, 255), 0.9))
        .light(SdfLight::new(Vec3::new(-5.0, 6.0, -2.0), Color::from_rgba8(150, 100, 255, 255), 0.5))
        .light(SdfLight::new(Vec3::new(0.0, 3.0, 5.0), Color::from_rgba8(100, 200, 180, 255), 0.3))
        .camera(SdfCamera::new(
...
fn build_ember_door() -> SdfScene {
    ...
    SdfScene::new()
        ...
        .object(SdfObject::new(
            SdfShape::Box { half_extents: Vec3::new(2.5, 3.0, 0.08) },
            Material::Solid {
        ...
        .object(SdfObject::new(
            SdfShape::Cylinder { radius: 0.2, half_height: 3.0 },
            Material::Solid { color: bone, reflectivity: 0.15, specular: 24.0 },
        ...
        .object(SdfObject::new(
            SdfShape::Cylinder { radius: 0.03, half_height: 2.5 },
            Material::Solid {
                color: Color::from_rgba8(160, 140, 100, 255),
                reflectivity: 0.3, specular: 32.0,
            },
...
fn build_nested_glass() -> SdfScene {
    ...
    let mut scene = SdfScene::new()
        ...
        .object(SdfObject::new(
            SdfShape::Mandelbulb { power: 7.0, iterations: 10 },
            Material::Rainbow {
                saturation: 1.0,
                lightness: 0.55,
                hue_offset: 0.0,
    ...
    let orbit_data: [(f32, f32, Vec3, Color); 3] = [
        (3.5, 0.04, Vec3::new(1.0, 0.3, 0.0), Color::from_rgba8(96, 165, 250, 255)),
        (4.0, 0.03, Vec3::new(0.0, 0.5, 1.0), Color::from_rgba8(167, 139, 250, 255)),
        (4.5, 0.025, Vec3::new(0.7, 1.0, 0.3), Color::from_rgba8(74, 222, 128, 255)),
    ];
    for (major, minor, axis, color) in &orbit_data {
        scene = scene.object(SdfObject::new(
            SdfShape::Torus { major: *major, minor: *minor },
            Material::Solid { color: *color, reflectivity: 0.4, specular: 64.0 },
        ).at(Vec3::new(0.0, 3.0, 0.0))
         .rotate(*axis, 0.8));
    }
    ...
    scene
        ...
        // Cool fill from left
        .light(SdfLight::new(Vec3::new(-8.0, 6.0, 4.0), Color::from_rgba8(120, 140, 255, 255), 0.5))
        // Warm accent from behind — highlights shell edges
        .light(SdfLight::new(Vec3::new(0.0, 4.0, -8.0), Color::from_rgba8(255, 200, 130, 255), 0.4))
        .camera(SdfCamera::new(
...
fn build_mirror_corridor() -> SdfScene {
    ...
    let mut scene = SdfScene::new()
        ...
        .object(SdfObject::new(
            SdfShape::Box { half_extents: Vec3::new(0.05, 5.0, 8.0) },
            Material::Solid {
    ...
    for (z, color) in pillar_z.iter().zip(pillar_colors.iter()) {
        ...
        scene = scene
            .object(SdfObject::new(
                SdfShape::Cylinder { radius: 0.08, half_height: 2.5 },
                Material::Solid { color: *color, reflectivity: 0.3, specular: 48.0 },
    ...
    scene = scene.object(SdfObject::new(
        SdfShape::Torus { major: 3.0, minor: 0.06 },
        Material::Solid {
    ...
    scene
        ...
        .light(SdfLight::new(Vec3::new(2.0, 3.0, 5.0), Color::from_rgba8(255, 200, 150, 255), 0.5))
        // Cool accent from the other end
        .light(SdfLight::new(Vec3::new(-1.0, 2.0, -6.0), Color::from_rgba8(100, 150, 255, 255), 0.4))
        // Purple uplighting for drama
        .light(SdfLight::new(Vec3::new(0.0, 0.2, 0.0), Color::from_rgba8(180, 100, 255, 255), 0.2))
        .camera(SdfCamera::new(
