...
fn option_a_crystal_blue() -> PixelCanvas {
    ...
    // Title bar
    let c = c.rect(0.0, 0.0, W as f32, 80.0).fill(Color::WHITE).done();

    // "scry" wordmark area in primary
    let c = c
        .rect(40.0, 20.0, 120.0, 40.0)
        .fill(primary)
        .corner_radius(8.0)
        .done();
    ...
    let c = c
        .rect(start_x + sw + gap, label_y, sw, label_h)
        .fill(secondary)
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 2.0, label_y, sw, label_h)
        .fill(accent)
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 3.0, label_y, sw, label_h)
        .fill(dark)
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 4.0, label_y, sw, label_h)
        .fill(muted)
        .done();
    ...
    // Card body lines
    let c = c
        .rect(56.0, 456.0, 300.0, 12.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(56.0, 478.0, 260.0, 12.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(56.0, 500.0, 280.0, 12.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    ...
    // Card button
    let c = c
        .rect(56.0, 536.0, 120.0, 36.0)
        .fill(primary)
        .corner_radius(8.0)
        .done();

    // Accent circle
    let c = c.circle(330.0, 560.0, 24.0).fill(secondary).done();

    // Dark panel UI mock
    let c = c
        .rect(420.0, 400.0, 350.0, 250.0)
        .fill(dark)
        .corner_radius(16.0)
        .done();
    ...
    // Panel elements
    let c = c
        .rect(440.0, 420.0, 200.0, 16.0)
        .fill(Color::from_rgba8(255, 255, 255, 200))
        .corner_radius(3.0)
        .done();
    let c = c
        .rect(440.0, 452.0, 310.0, 8.0)
        .fill(Color::from_rgba8(255, 255, 255, 60))
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(440.0, 468.0, 280.0, 8.0)
        .fill(Color::from_rgba8(255, 255, 255, 60))
        .corner_radius(2.0)
        .done();
    ...
    let bar_base = 580.0;
    let c = c
        .rect(440.0, bar_base, 30.0, -80.0)
        .fill(primary)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(480.0, bar_base, 30.0, -120.0)
        .fill(secondary)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(520.0, bar_base, 30.0, -60.0)
        .fill(accent)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(560.0, bar_base, 30.0, -100.0)
        .fill(primary)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(600.0, bar_base, 30.0, -140.0)
        .fill(secondary)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(640.0, bar_base, 30.0, -90.0)
        .fill(accent)
        .corner_radius(4.0)
        .done();
...
fn option_b_obsidian_electric() -> PixelCanvas {
    ...
    // Top bar surface
    let c = c.rect(0.0, 0.0, W as f32, 80.0).fill(surface).done();
    ...
    let c = c
        .rect(start_x + sw + gap, label_y, sw, 6.0)
        .fill(secondary)
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 2.0, label_y, sw, 6.0)
        .fill(accent)
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 3.0, label_y, sw, 6.0)
        .fill(Color::from_rgba8(71, 85, 105, 200))
        .done();
    let c = c
        .rect(start_x + (sw + gap) * 4.0, label_y, sw, 6.0)
        .fill(Color::from_rgba8(51, 65, 85, 200))
        .done();
    ...
    let c = gradient_bar(
        ...
        W as f32 - 80.0,
        40.0,
        &[(0.0, primary), (0.5, secondary), (1.0, accent)],
    );
    ...
    // Card body lines
    let c = c
        .rect(56.0, 456.0, 300.0, 10.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(56.0, 476.0, 260.0, 10.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(56.0, 496.0, 280.0, 10.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    ...
    let c = c
        .rect(56.0, 530.0, 120.0, 36.0)
        .fill(primary)
        .corner_radius(8.0)
        .done();

    // Accent indicator
    let c = c.circle(330.0, 560.0, 20.0).fill(accent).done();
    ...
    let c = c
        ...
        .done();
    let c = c
        .rect(480.0, bar_base, 30.0, -130.0)
        .fill(secondary)
        .corner_radius(4.0)
        .done();
    let c = c
        ...
        .done();
    let c = c
        .rect(560.0, bar_base, 30.0, -110.0)
        .fill(primary)
        .corner_radius(4.0)
        .done();
    let c = c
        .rect(600.0, bar_base, 30.0, -150.0)
        .fill(secondary)
        .corner_radius(4.0)
        .done();
    let c = c
    ...
    // Floating glow orbs
    let c = c
        .circle(850.0, 500.0, 60.0)
        .fill(Color::from_rgba8(56, 189, 248, 20))
        .done();
    let c = c
        .circle(1050.0, 450.0, 80.0)
        .fill(Color::from_rgba8(167, 139, 250, 15))
        .done();
    let c = c
        .circle(1100.0, 600.0, 50.0)
        .fill(Color::from_rgba8(251, 191, 36, 15))
        .done();

    // Subtle grid lines
    let c = c
        .line(800.0, 400.0, 1160.0, 400.0)
        .color(Color::from_rgba8(255, 255, 255, 15))
        .width(1.0)
        .done();
    let c = c
        .line(800.0, 500.0, 1160.0, 500.0)
        .color(Color::from_rgba8(255, 255, 255, 15))
        .width(1.0)
        .done();
    let c = c
        .line(800.0, 600.0, 1160.0, 600.0)
        .color(Color::from_rgba8(255, 255, 255, 15))
        .width(1.0)
        .done();
...
fn option_c_prismatic() -> PixelCanvas {
    ...
    // Top bar
    let c = c.rect(0.0, 0.0, W as f32, 80.0).fill(deep_navy).done();
    ...
    let c = gradient_bar(
        ...
        120.0,
        40.0,
        &[(0.0, prism_blue), (0.5, prism_violet), (1.0, prism_red)],
    );
    ...
    // Card content
    let c = c
        .rect(56.0, 430.0, 200.0, 18.0)
        .fill(Color::from_rgba8(255, 255, 255, 200))
        .corner_radius(3.0)
        .done();
    let muted = Color::from_rgba8(100, 116, 139, 255);
    let c = c
        .rect(56.0, 462.0, 300.0, 10.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    let c = c
        .rect(56.0, 480.0, 260.0, 10.0)
        .fill(muted)
        .corner_radius(2.0)
        .done();
    ...
    let c = gradient_bar(
        ...
        140.0,
        36.0,
        &[(0.0, prism_blue), (1.0, prism_violet)],
    );

    // Chart card
    let c = c
        .rect(420.0, 410.0, 350.0, 240.0)
    ...
    let c = gradient_bar(
        ...
        350.0,
        4.0,
        &[(0.0, prism_green), (0.5, prism_yellow), (1.0, prism_orange)],
    );
    ...
    let c = c
        .line(
            prism_cx,
            prism_cy - prism_r,
            prism_cx - prism_r * 0.866,
            prism_cy + prism_r * 0.5,
        )
    ...
    // Incoming light beam
    let c = c
        .line(800.0, prism_cy, prism_cx - 40.0, prism_cy)
    ...
    for (i, &color) in rainbow.iter().enumerate() {
        ...
        c = c
            .line(prism_cx + 40.0, prism_cy, end_x, end_y)
            .color(color)
            .width(2.0)
            .done();
    }
    ...
    let c = c
        .circle(880.0, 500.0, 50.0)
        .fill(Color::from_rgba8(96, 165, 250, 20))
        .done();
    let c = c
        .circle(1050.0, 550.0, 70.0)
        .fill(Color::from_rgba8(167, 139, 250, 15))
        .done();
    let c = c
        .circle(1100.0, 420.0, 40.0)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "\n🎨 All three brand options rendered to {}",
        out_dir.display()
    );
