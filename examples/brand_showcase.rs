...
mod brand {
    ...
    pub const BG: Color        = Color { r: 0.059, g: 0.078, b: 0.137, a: 1.0 }; // #0F1423
    pub const SURFACE: Color   = Color { r: 0.118, g: 0.161, b: 0.231, a: 1.0 }; // #1E293B
    pub const RED: Color       = Color { r: 0.973, g: 0.443, b: 0.443, a: 1.0 }; // #F87171
    pub const ORANGE: Color    = Color { r: 0.984, g: 0.573, b: 0.235, a: 1.0 }; // #FB923C
    pub const YELLOW: Color    = Color { r: 0.980, g: 0.800, b: 0.082, a: 1.0 }; // #FACC15
    pub const GREEN: Color     = Color { r: 0.290, g: 0.871, b: 0.502, a: 1.0 }; // #4ADE80
    pub const BLUE: Color      = Color { r: 0.376, g: 0.647, b: 0.980, a: 1.0 }; // #60A5FA
    pub const VIOLET: Color    = Color { r: 0.655, g: 0.545, b: 0.980, a: 1.0 }; // #A78BFA
    pub const WHITE: Color     = Color { r: 0.95, g: 0.95, b: 0.97, a: 1.0 };
    pub const MUTED: Color     = Color { r: 0.392, g: 0.455, b: 0.545, a: 1.0 }; // #647888
}

/// Build a custom prismatic theme for charts.
fn prismatic_theme() -> Theme {
    Theme::dark()
        .with_background(brand::BG)
...
fn scrying_mirror() -> PixelCanvas {
    ...
    let mut c = PixelCanvas::new(w, h)
        .background(Color::from_rgba8(10, 14, 28, 255));

    // Subtle grid lines
    for i in 0..20 {
        let y = i as f32 * (h as f32 / 20.0);
        c = c.line(0.0, y, w as f32, y)
    ...
    for (radius, color, alpha) in spectrum.iter() {
        let c_alpha = Color { a: *alpha as f32 / 255.0, ..*color };
        c = c.circle(cx, cy, *radius)
            .fill(c_alpha)
            .done();
    }
    ...
    // Central bright core
    c = c.circle(cx, cy, 30.0)
        .fill(Color::from_rgba8(255, 255, 255, 15))
        .done();
    c = c.circle(cx, cy, 15.0)
        .fill(Color::from_rgba8(255, 255, 255, 25))
        .done();

    // Specular highlight
    c = c.circle(cx - 50.0, cy - 60.0, 25.0)
        .fill(Color::from_rgba8(255, 255, 255, 20))
        .done();

    // Data visualization rays emanating from orb
    let ray_colors = [brand::RED, brand::ORANGE, brand::YELLOW, brand::GREEN, brand::BLUE, brand::VIOLET];
    for (i, &color) in ray_colors.iter().enumerate() {
        ...
        c = c.line(x1, y1, x2, y2)
            .color(c_alpha)
            .width(2.0)
            .done();

        // Data point at end of ray
        c = c.circle(x2, y2, 5.0)
            .fill(color)
            .done();
    }
    ...
    c = c.gradient(0.0, 0.0, w as f32, 3.0)
...
fn ml_dashboard() -> SubplotGrid {
    ...
    let latency = Charts::scatter(&p50, &p95)
        ...
        .subtitle("p50 vs p95 — all sub-microsecond")
        .x_label("p50 (µs)")
        .y_label("p95 (µs)")
        .theme(theme.clone())
        .build();
...
fn chart_gallery() -> SubplotGrid {
    ...
    // Candlestick
    let ohlc: Vec<OhlcEntry> = (0..20).map(|i| {
    ...
    // Heatmap
    let hm_data: Vec<Vec<f64>> = (0..10).map(|r| {
        (0..10).map(|c| {
            let x = r as f64 / 10.0;
            let y = c as f64 / 10.0;
...
fn architecture_visual() -> PixelCanvas {
    ...
    for (_, x, y, bw, bh, color) in boxes.iter() {
        ...
        // Fake text lines
        c = c.rect(*x + 16.0, *y + 30.0, *bw * 0.7, 12.0)
            .fill(Color::from_rgba8(255, 255, 255, 180))
            .corner_radius(2.0)
            .done();
        c = c.rect(*x + 16.0, *y + 52.0, *bw * 0.5, 8.0)
            .fill(Color::from_rgba8(255, 255, 255, 60))
            .corner_radius(2.0)
            .done();
        c = c.rect(*x + 16.0, *y + 66.0, *bw * 0.6, 8.0)
            .fill(Color::from_rgba8(255, 255, 255, 60))
            .corner_radius(2.0)
            .done();

        // LOC badge
        c = c.rect(*x + 16.0, *y + *bh - 36.0, 60.0, 22.0)
    ...
    for (x, _, color) in metric_boxes.iter() {
        c = c.rect(*x, metrics_y, 160.0, 80.0)
        ...
        c = c.rect(*x + 20.0, metrics_y + 18.0, 80.0, 18.0)
            .fill(Color { a: 0.9, ..*color })
            .corner_radius(3.0)
            .done();

        // Label placeholder
        c = c.rect(*x + 20.0, metrics_y + 48.0, 100.0, 10.0)
    ...
    for (i, (_, val, color)) in bar_labels.iter().enumerate() {
        ...
        // Bar
        c = c.rect(x, bottom_y + 140.0 - bar_h, bar_width, bar_h)
            .fill(Color { a: 0.8, ..*color })
            .corner_radius(6.0)
            .done();

        // Label
        c = c.rect(x + 20.0, bottom_y + 150.0, 60.0, 10.0)
    ...
    c = c.gradient(0.0, h as f32 - 3.0, w as f32, 3.0)
        .linear(Point::new(0.0, 0.0), Point::new(w as f32, 0.0))
